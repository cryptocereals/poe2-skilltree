...
function SkillsStep({ skills, setSkills }: Props) {
  ...
  const addSkill = () => setSkills([...skills, { id: "", support_skills: [] }]);
  const removeSkill = (i: number) => setSkills(skills.filter((_, idx) => idx !== i));
  const addSupport = (i: number) => patch(i, (s) => ({ ...s, support_skills: [...(s.support_skills ?? []), { id: "" }] }));
  const setSupport = (i: number, j: number, fn: (x: { id: string; level_interval?: number | number[] }) => typeof x) =>
    patch(i, (s) => ({ ...s, support_skills: (s.support_skills ?? []).map((x, k) => (k === j ? fn(x) : x)) }));
  const removeSupport = (i: number, j: number) =>
    patch(i, (s) => ({ ...s, support_skills: (s.support_skills ?? []).filter((_, k) => k !== j) }));
